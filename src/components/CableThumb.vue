<template>
  <li class="relative group my-12">
    ...
    <ul class="text-3xs px-2 leading-normal align-baseline">
      ...
      <li><span>adopted by</span> <template v-if="owner"><a class="text-2xs leading-none border-b border-dashed mouse_hover_border-solid font-bold" :href="$store.getters.marketplaceLink({ account: owner })" target="_blank" rel="noopener noreferrer"><Addr :address="owner"></Addr></a></template><template v-else><Observer class="inline" @visible="getOwner"><span class="animate-pulse">...</span></Observer></template></li>
