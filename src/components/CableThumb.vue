<template>
  <li class="relative">
    <figure class="aspect-square">
      <a class="block" :href="$store.getters.marketplaceLink({ token: props.id })" target="_blank" rel="noopener noreferrer">
        <CableImage :id="props.id"></CableImage>
      </a>
      <p class="text-xs px-2">adopted by <template v-if="owner"><a class="border-b border-dashed mouse_hover_border-solid font-bold" :href="$store.getters.marketplaceLink({ account: owner })" target="_blank" rel="noopener noreferrer"><Addr :address="owner"></Addr></a></template><template v-else><Observer class="inline" @visible="getOwner"><span class="animate-pulse">...</span></Observer></template></p>
    </figure>
  </li>
</template>
